...
-- To get your user_id, run: SELECT id FROM auth.users WHERE email = 'your-email@example.com';

-- OR use this dynamic version that auto-detects your user:
-- DELETE FROM categories WHERE user_id = auth.uid(); -- Optional: clear existing
-- Then run the INSERT statements below with auth.uid() instead of 'YOUR_USER_ID'
...
VALUES (
  auth.uid(), -- Auto-uses logged-in user
  'Grocery',
...
-- Run this to verify all categories were inserted:
-- SELECT name, icon, array_length(keywords_en, 1) as en_count, array_length(keywords_zh, 1) as zh_count 
-- FROM categories 
